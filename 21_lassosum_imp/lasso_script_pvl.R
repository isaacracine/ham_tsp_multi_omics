...
output_prefix = "apr_27_ham_ss_pvl"
...
target.pheno <- fread("input/pheno_pvl.txt")

#our names are messed up, rename
colnames(target.pheno) = c("FID", "IID", "pheno")

target.pheno$pheno <- log10(target.pheno$pheno)
...
bfile_ref <- "input/no_amr_only_merged_snps"
