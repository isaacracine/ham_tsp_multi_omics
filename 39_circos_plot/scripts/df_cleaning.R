...
### TO-DO
#update readme
#add scale for GWAS and transcript?
...
df <- data.frame(Gene=unlist(mget(x = tr_res$ID,envir = illuminaHumanv4SYMBOL)))
stat1 <- subset(df, Gene == 'STAT1')
stat1_info <- tr_res[tr_res$ID %in% rownames(stat1), ]
# all are negative!
...
circos.par(
  ...
  track.margin = c(0.02, 0.02),  # Adjust margins between tracks (smaller values for less space)
...
png("../output/circos_plot_testing.png", width = 800, height = 800)
...
circos.track(
  ...
  panel.fun = function(x, y) {
    circos.text(
      ...
      CELL_META$ylim[2] + mm_y(2),  # Position the text above the track
...
lgd = Legend(
  ...
  labels_gp = gpar(fontsize = 10),  # Adjust label font size
  at = c(5e-8, 0.05)
)
...
create_manhattan_track <- function(gwas_data, p_value_column, track_color, threshold = -log10(5e-8), track_height = 0.2) {
  ...
  circos.trackPoints(factors = as.factor(gwas_data$CHR), 
                     x = gwas_data$POS, 
                     y = gwas_data$log_p, 
                     pch = 16, col = track_color, cex = 0.01)
...
create_manhattan_track(gwas_data, p_value_column = "P", track_color = "blue")
...
for (i in seq_len(nrow(string_int))) {
  circos.link(
    ...
    lwd = string_int$combined_score[i]               # Line width scaled by score
  )
}
